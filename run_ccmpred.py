...
def parse_args():
    ...
    grp_gp.add_argument("--max-gap-pos",  dest="max_gap_pos",  default=100, type=int, help="Ignore alignment positions with >X% gaps. [default: %(default)s == no removal of gaps]")
    grp_gp.add_argument("--max-gap-seq",  dest="max_gap_seq",  default=100, type=int, help="Remove sequences with >X% gaps. [default: %(default)s == no removal of sequences]")
    ...
    grp_pl = parser.add_argument_group("Plotting Options")
    grp_pl.add_argument("--plot-aln-stats",             dest="plot_alnstats",        default=False, action="store_true",  help="Plot observed vs model alignment statistics as interactive html")
    grp_pl.add_argument("--plot-aln-aa-distribution",   dest="plot_aln_aadistribution", default=False, action="store_true",  help="Plot amino acid distribution in alignment asinteractive html")
    grp_pl.add_argument("--plot-opt-progress",          dest="plot_opt_progress",            default=False, action="store_true",  help="Plot optimization progress as interactive html. [default: %(default)s]")
    ...
    args.plot_aln_aadistribution_file = None
    if args.plot_aln_aadistribution:
        args.plot_aln_aadistribution_file = ".".join(args.matfile.split(".")[:-1]) + ".aln_aa_distribution.html"

    args.plot_alnstats_file = None
    if args.plot_alnstats:
        args.plot_alnstats_file = ".".join(args.matfile.split(".")[:-1]) + ".empirical_vs_model_alignment_stats.html"
...
def main():
    ...
    # plot amino acid distribution per position of alignment
    if opt.plot_aln_aadistribution:
        plotting.plot_alignment(
            ccm.pseudocounts.counts[0],
            "Amino Acid Distribution in Alignment for {0} (N={1}, L={2}, diversity={3})".format(
                ccm.protein, ccm.N, ccm.L, np.round(ccm.diversity, decimals=3)),
            opt.plot_aln_aadistribution_file
        )
